...
import styles from './ListTimesharesAscByPrice.module.scss';
...
let listTimeshares = [];
function ListTimesharesAscByPrice() {
    const [timeshares, setTimeshares] = useState([]);
    useEffect(() => {
        fetch('http://localhost:8080/api/v1/timeshare/getAllOrderByPriceAsc')
            .then((res) => res.json())
            .then((timeshare) => {
...
export default ListTimesharesAscByPrice;
