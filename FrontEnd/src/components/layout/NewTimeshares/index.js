...
import styles from './NewTimeshares.module.scss';
...
function NewTimeshares() {
    ...
    if (timeshares.length > 0) {
        timesharesSale = timeshares.filter((p, index) => p.id >= 20 && p.id <= 29);
    }
    return (
        <div className={cx('container')}>
            <div className={cx('wrapper')}>
                <div className={cx('text')}>
                    <h2 className={cx('title')}>New Timeshares</h2>
