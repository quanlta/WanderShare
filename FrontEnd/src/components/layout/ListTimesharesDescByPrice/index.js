...
import styles from './ListTimesharesDescByPrice.module.scss';
...
let listTimeshares = [];
function ListTimesharesDescByPrice() {
    const [timeshares, setTimeshares] = useState([]);
    useEffect(() => {
        fetch('http://localhost:8080/api/v1/timeshare/getAllOrderByPriceDesc')
            .then((res) => res.json())
            .then((timeshare) => {
...
export default ListTimesharesDescByPrice;
