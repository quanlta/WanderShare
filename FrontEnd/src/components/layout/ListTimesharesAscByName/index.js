...
import styles from './ListTimesharesAscByName.module.scss';
...
let listTimeshares = [];
function ListTimesharesAscByName() {
    const [timeshares, setTimeshares] = useState([]);
    useEffect(() => {
        fetch('http://localhost:8080/api/v1/timeshare/getAllOrderByNameAsc')
            .then((res) => res.json())
            .then((timeshare) => {
...
export default ListTimesharesAscByName;
